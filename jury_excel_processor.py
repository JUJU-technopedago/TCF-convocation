...
class JuryExcelProcessor:
            ...
            for sheet_name in excel_file.sheet_names:
                if sheet_name.startswith('Niveau '):
                    ...
                    for i in range(1, len(df)):
                        ...
                        # Vérifier s'il s'agit d'une ligne de candidat et pas de l'en-tête
                        if pd.notna(row[0]) and pd.notna(row[1]) and pd.notna(row[2]):
                            # Vérifier si c'est une ligne d'en-tête (plusieurs méthodes de détection)
                            first_col = str(row[0]).strip().lower()
                            second_col = str(row[1]).strip().lower()
                            third_col = str(row[2]).strip().lower()
                            ...
                            if is_header:
                                ...
                                print(f"En-tête détectée et ignorée: {row[0]} {row[1]} {row[2]}")
                                continue
    ...
    def _calculate_end_time(self, start_time, duration):
        """Calcule l'heure de fin approximative avec gestion sécurisée des types"""
        if not start_time:
            return ''
            
        # Assurer que start_time est une chaîne
        start_time = self._ensure_string(start_time)
            
        # Retourner une valeur par défaut sans traiter les formats d'heure
        # Utiliser simplement une approximation en fonction du niveau DELF
        if isinstance(duration, str):
            if '1h20' in duration:
                return '12:00'  # Valeur approximative pour A1
            elif '1h40' in duration:
                return '12:00'  # Valeur approximative pour A2
            elif '1h45' in duration:
                return '12:30'  # Valeur approximative pour B1
            elif '2h30' in duration:
                return '13:00'  # Valeur approximative pour B2
            elif '4h' in duration:
                return '14:00'  # Valeur approximative pour C1
            elif '3h30' in duration:
                return '14:00'  # Valeur approximative pour C2
