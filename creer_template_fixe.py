...
# -*- coding: utf-8 -*-
"""
Créer une version corrigée du template HTML directement dans le répertoire templates
"""
...
def create_fixed_template_in_templates():
    """
    Crée une version corrigée du template HTML directement dans le répertoire templates
    """
    ...
    backup_template = os.path.join(os.path.dirname(os.path.abspath(__file__)), 
                               "templates", "convocation_delf_template_modele_original.html")
    
    # Vérifier si la sauvegarde existe déjà
    if not os.path.exists(backup_template):
        shutil.copy2(source_template, backup_template)
        print(f"Sauvegarde du template original créée: {backup_template}")
    
    # Nouveau fichier avec le suffixe _fixed
    fixed_template = os.path.join(os.path.dirname(os.path.abspath(__file__)), 
                               "templates", "convocation_delf_template_fixed.html")
    ...
    # Fixer la taille du texte dans le cadre titre
    fixed_content = fixed_content.replace('font-size: 35pt;', 'font-size: 15pt;')
    fixed_content = fixed_content.replace('font-size: 26pt;', 'font-size: 15pt;')
    fixed_content = fixed_content.replace('font-size: 20pt;', 'font-size: 15pt;')
    ...
    print(f"Template fixé créé dans le répertoire templates: {fixed_template}")
    
    return True

if __name__ == "__main__":
    success = create_fixed_template_in_templates()
    if success:
        print("✅ Le template corrigé a été créé avec succès!")
    else:
        print("❌ Une erreur est survenue lors de la création du template corrigé.")